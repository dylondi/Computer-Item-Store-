...
public class Analyser {
    ...
    public String performAnalysis(Inventory inventory) {
        ...
        String returned = sortAndPrint(computerComponents) +
            "Average price of all computer components: " + computeAvgPriceOfAllComponents(computerComponents) + "\n" +
    ...
    double computeAvgPriceOfAllComponents(List<ComputerComponent> computerComponents) {
        return Math.round(computerComponents.stream()
                .map(c -> c.getPrice())
                .mapToDouble(num -> num)
                .sum() / computerComponents.size() * 100.0) / 100.0;
    }
    ...
    String cheapestComponent(List<ComputerComponent> computerComponents) {
        List<ComputerComponent> cheapestComponentObj = computerComponents.stream()
                .sorted(Comparator.comparing(ComputerComponent::getPrice))
                .collect(Collectors.toList());
    ...
    String getMostExpensiveOfEachCategory(List<ComputerComponent> computerComponents) {
        return computerComponents.stream()
                .collect(Collectors.groupingBy(ComputerComponent::getCategory, Collectors.maxBy(Comparator.comparingInt(ComputerComponent::getPrice))))
                .toString();
    }

    String reportOfQuantityOfComponentsByCategory(List<ComputerComponent> computerComponents) {
        return computerComponents.stream()
                .collect(Collectors.groupingBy(ComputerComponent::getCategory, Collectors.summingInt(ComputerComponent::getQuantity)))
                .toString();
    }

    String reportOfQuantityOfComponentsByCategoryAndBrand(List<ComputerComponent> computerComponents) {
        return computerComponents.stream()
                .collect(Collectors.groupingBy(ComputerComponent::getCategory, Collectors.groupingBy(ComputerComponent::getBrand, Collectors.summingInt(ComputerComponent::getQuantity))))
                .toString();
    }
}
